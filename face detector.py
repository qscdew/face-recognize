...
# 调用人脸分类器，要根据实际路径调整3
face_detector = cv2.CascadeClassifier(r'D:/opencv-master/opencv-master/data/haarcascades/haarcascade_frontalface_default.xml')  #待更改
face_detector2 = cv2.CascadeClassifier(r'D:/opencv-master/opencv-master/data/haarcascades/haarcascade_eye.xml')
# 为即将录入的脸标记一个id
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        eyes = face_detector2.detectMultiScale(gray, 1.3, 5)
        for (eyex, eyey, eyew, eyeh) in eyes:
            cv2.rectangle(img, (eyex, eyey), (eyex + eyew, eyey + eyew), (0, 255, 0), 2)
        # 成功框选则样本数增加
        count += 1
